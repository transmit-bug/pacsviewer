 ...
 *   - Patients (20)
 *   - Devices & Adapters
 *   - Studies (40) with Series (80+) and Images (200+)
 *   - Report Templates (6)
...
async function seed() {
  ...
  await db.insert(deviceAdapters).values([
    {
      ...
      type: 'dicom',
      status: 'running',
      config: JSON.stringify({ aeTitle: 'PACSVIEWER', port: 11112, storePath: './data/dicom' }),
      enabled: true,
      imageCount: 0,
      createdAt: dateAgo(300),
      updatedAt: dateAgo(1),
    },
    {
      ...
      type: 'rest',
      status: 'running',
      config: JSON.stringify({ apiKey: 'demo-api-key-2024', maxFileSize: 100 * 1024 * 1024 }),
      enabled: true,
      imageCount: 0,
      createdAt: dateAgo(200),
      updatedAt: dateAgo(5),
  ...
  await db.insert(reportTemplates).values([
    {
      ...
      layout: JSON.stringify({ columns: 2, sections: ['基本信息', '测量数据', '诊断'] }),
      isSystem: true,
    ...
    },
    {
      ...
      layout: JSON.stringify({ columns: 1, sections: ['基本信息', '眼底所见', '诊断'] }),
      isSystem: true,
      createdBy: userIds.admin,
      createdAt: dateAgo(365),
      updatedAt: dateAgo(30),
    },
    {
      ...
      layout: JSON.stringify({ columns: 1, sections: ['基本信息', '造影数据', '诊断'] }),
      isSystem: true,
      createdBy: userIds.admin,
      createdAt: dateAgo(365),
      updatedAt: dateAgo(30),
    },
    {
      ...
      layout: JSON.stringify({ columns: 2, sections: ['基本信息', '视野指数', '诊断'] }),
      isSystem: true,
      createdBy: userIds.admin,
      createdAt: dateAgo(365),
      updatedAt: dateAgo(30),
    },
    {
      ...
      layout: JSON.stringify({ columns: 2, sections: ['基本信息', '血管分析', '诊断'] }),
      isSystem: true,
      createdBy: userIds.admin,
      createdAt: dateAgo(200),
      updatedAt: dateAgo(30),
    },
    {
      ...
      layout: JSON.stringify({ columns: 1, sections: ['病史', '检查', '诊断', '治疗'] }),
      isSystem: true,
      createdBy: userIds.admin,
      createdAt: dateAgo(365),
      updatedAt: dateAgo(30),
  ...
  for (const pid of patientIds) {
    ...
    for (let s = 0; s < numStudies; s++) {
      ...
      // Create 1-3 series per study
      const numSeries = 1 + Math.floor(Math.random() * 3);
      for (let si = 0; si < numSeries; si++) {
        const seriesId = uuid();
        const imageCount = 2 + Math.floor(Math.random() * 4); // 2-5 images per series
        ...
        for (let ii = 0; ii < imageCount; ii++) {
          const imageId = uuid();
          const width = pick([512, 768, 1024, 2048]);
          const height = pick([512, 768, 1024, 2048]);
          const format = modality === 'VF' ? 'png' as const : pick(['jpeg', 'png', 'dicom'] as const);

          await db.insert(images).values({
            id: imageId,
            seriesId,
            instanceNumber: ii + 1,
            filePath: `placeholder/${imageId}.${format}`,
            fileSize: Math.floor(Math.random() * 5000000) + 100000,
            fileHash: `hash_${imageId.slice(0, 8)}`,
            format,
            width,
            height,
            bitsAllocated: modality === 'OCT' ? 16 : 8,
            thumbnailPath: null,
            metadata: JSON.stringify({
              modality,
              bodyPart: pick(bodyParts),
              pixelSpacing: [0.01, 0.01],
            }),
            createdAt: studyDate,
          });
          totalImages++;
  ...
  console.log(`✅ Studies (${studyIds.length}), Series, Images (${totalImages}) created`);
  ...
  for (let i = 0; i < reportsToCreate; i++) {
    ...
    if (status !== 'draft') {
      await db.insert(reportVersions).values({
        ...
        version: 1,
        status: 'draft',
        content: JSON.stringify({ ...content, impression: '初稿' }),
        changeNotes: '初稿创建',
        createdBy,
        createdAt,
      });
  ...
  for (const img of sampleImages.slice(0, 5)) {
    ...
    for (let a = 0; a < 2; a++) {
      await db.insert(annotations).values({
        ...
        style: JSON.stringify({ color: '#ff0000', lineWidth: 2, fontSize: 14 }),
    ...
    await db.insert(comparisons).values([
      {
        ...
        name: 'OCT 前后对比',
        type: 'side_by_side',
        config: JSON.stringify({ layout: 'horizontal', syncScroll: true }),
      ...
      },
      {
        ...
        name: '眼底彩照叠加分析',
        type: 'overlay',
        config: JSON.stringify({ opacity: 0.5, blendMode: 'difference' }),
        imageIds: JSON.stringify([]),
        isFavorite: false,
        createdBy: userIds.doctor1,
        createdAt: dateAgo(7),
      ...
      },
      {
        ...
        name: 'FFA 时间序列',
        type: 'slider',
        config: JSON.stringify({ direction: 'horizontal' }),
        imageIds: JSON.stringify([]),
        isFavorite: false,
  ...
  await db.insert(systemSettings).values([
    {
      ...
      category: 'general',
      key: 'siteName',
      value: JSON.stringify('PACS Viewer 眼科影像管理系统'),
      description: '系统名称',
      updatedAt: dateAgo(365),
    },
    {
    ...
    },
    {
      ...
      category: 'dicom',
      key: 'aeTitle',
      value: JSON.stringify('PACSVIEWER'),
    ...
    },
    {
      ...
      category: 'dicom',
      key: 'storePath',
      value: JSON.stringify('./data/dicom'),
    ...
    },
    {
      ...
      category: 'storage',
      key: 'maxFileSize',
      value: JSON.stringify(100 * 1024 * 1024),
    ...
    },
    {
      ...
      category: 'storage',
      key: 'allowedFormats',
      value: JSON.stringify(['dicom', 'jpeg', 'png', 'tiff', 'bmp']),
      description: '允许的图像格式',
      updatedAt: dateAgo(300),
    },
    {
      ...
      category: 'storage',
      key: 'thumbnailSize',
      value: JSON.stringify({ width: 256, height: 256 }),
      description: '缩略图尺寸',
      updatedAt: dateAgo(300),
    },
    {
