 ...
 *   - System Settings
 *
 * Note: Images are NOT generated by this script.
 * Use scripts/seed-hrf.ts to import real fundus images from HRF dataset.
 *
...
async function seed() {
  ...
  for (const pid of patientIds) {
    ...
    for (let s = 0; s < numStudies; s++) {
      ...
      // Create 1-3 series per study (without images - use seed-hrf.ts for real images)
      const numSeries = 1 + Math.floor(Math.random() * 3);
      for (let si = 0; si < numSeries; si++) {
        const seriesId = uuid();
        const imageCount = 0; // Images will be added by seed-hrf.ts
  ...
  console.log(`✅ Studies (${studyIds.length}), Series (${totalSeries}) created`);
  console.log('   Note: Use scripts/seed-hrf.ts to import real fundus images');
  ...
  console.log(`  Images:          0 (use db:seed-hrf to import)`);
